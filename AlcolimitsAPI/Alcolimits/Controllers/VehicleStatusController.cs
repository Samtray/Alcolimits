...
namespace Alcolimits.Controllers
{
    public class VehicleStatusController : ApiController
    {
        ...
        {
            List<ConVehicleStatus> statuses = new List<ConVehicleStatus>();
            DataTable dt = GetData("exec GetStatus");
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ConVehicleStatus status = new ConVehicleStatus
                {
                    id = Convert.ToInt32(dt.Rows[i]["id"]),
                    isOn = Convert.ToBoolean(dt.Rows[i]["isOn"]),
                    isDriving = Convert.ToBoolean(dt.Rows[i]["isDriving"]),
                    ranges = statusRanges(),
                    current = statusCurrent(Convert.ToString(dt.Rows[i]["status"]))
                };
            ...
            var json = new JavaScriptSerializer().Serialize(statuses);
        ...
        public HttpResponseMessage Get(int id)
        {
            ...
            DataTable dt = GetData(string.Format("exec GetStatusId '{0}'", id));
            //for (int i = 0; i < dt.Rows.Count; i++)
            //{
            ConVehicleStatus status = new ConVehicleStatus
            {
